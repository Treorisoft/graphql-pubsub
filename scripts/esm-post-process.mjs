...
async function buildEsm(filePath) {
  ...
  if (fileExt === 'js' || fileExt === 'ts') {
    ...
    for (const match of content.matchAll(/from (['"])(\.?\.\/[^\1]*)\1/g)) {
      const [statement, quoteChar, relImportPath] = match;
      ...
      try {
        ...
        // file import
        content = content.replace(statement, `from ${quoteChar}${relImportPath}.mjs${quoteChar}`);
      } catch {
        // directory import
        content = content.replace(
          statement,
          `from ${quoteChar}${relImportPath}/index.mjs${quoteChar}`,
        );
      }
    }
  }
